...
### Wikipedia風出力の方位反転と陸海塗り分け
- 原因: 正射投影のスクリーン座標でY反転しており北が下向きになっていた。陸海判定をポリゴン種別に寄せていたため、領海や内海が陸色になるケースがあった。
- 対応: 投影のY符号を修正し、分高面（最初に見つかったレイヤー）から陸マスクを生成して陸/海/選択領域をマスク合成で塗り分けるようにした。
- 調査で苦戦: 分高面レイヤーを複数重ねるとeven-oddで穴が発生するため、基準レイヤーを1つに絞る方針に変更した。

### 点線国境グループの分断
- 原因: 国境界レイヤーの点線が`<g>`内の3本（短い端点線2本＋点線1本）として出力され、個別抽出で端点が連結されず境界が欠落していた。
- 対応: `<g>`直下の`path`群をサンプリング後に結合し、端点距離の合計が最小になる順序（総当たり探索）で連結して1本として抽出するようにした（表示は変更しない）。
- 調査で苦戦: 点線判定が不安定だったため、結合ロジックは点線判定に依存しない方針に切り替えた。点線スナップは`isDashed`で判定しつつ、デバッグで点線ハイライトを入れて確認できるようにした。

### 複数選択時の領域色差
- 原因: 複数選択時もポリゴンごとの`fill`（ランダム色/タイプ色）が残り、選択内で色差が出ていた。
- 対応: 選択数が2以上のときは選択ポリゴンの`fill`を統一色（`#e8b86c`）に切り替え、選択変更時に`updatePolygonFills`で再適用するようにした。
- 調査で苦戦: なし。

### 球面投影の半球クリッピングで地平線弧が欠落

- 原因: `clipPolygonToHemisphere`/`clipPolygonVectors`がSutherland-Hodgman法で半球クリップする際、切断点間を直線（弦）で結んでおり、球の輪郭（z=0の単位円弧）に沿った補間点を追加していなかった。弦と弧の間の面積が欠落し、球の側面付近で多角形が途切れて見えた。
- 対応: `addHorizonArc`関数を追加し、半球の出口→入口間に短い方の弧に沿った補間点を挿入するようにした。ラップアラウンド（最後の出口→最初の入口）も正しく処理される。
- 調査で苦戦: なし。投影の数学（回転行列・射影のx/y入替）を検証し、交差点が投影空間の単位円上にあることを確認した上で弧の角度補間を実装した。

### 面積表示機能

- 目的: 選択ポリゴンの総面積（海含む）と陸域面積を表示する。
- 総面積・陸域面積ともにオフスクリーンCanvas2枚（選択ポリゴン・第一分高面）のピクセル重ね合わせで算出。
- 正距円筒図法は等積でないため、各ピクセルに`cos(緯度)`の補正を掛けて球面上の実面積を計算: `R² × cos(lat) × dλ × dφ`（R = viewBox.width / 2π）。
- 2D陸地パスは`state.landPaths2D`にキャッシュ（`collectLandPaths2D`で`collectLandVectorPaths`と同じサンプリングだが3D変換せず2D座標を保持）。
- 単位: 赤道長 = viewBox.width として、ユーザーがm/kmを付与して解釈する。
- Canvas解像度: バウンディングボックスの長辺を800pxに正規化。
