...
- 総面積: 各ポリゴンの`area`（Shoelace公式、SVG座標²）を合計。
- 陸域面積: オフスクリーンCanvas2枚で選択ポリゴンと第一分高面の重なりをピクセル計算。`totalArea * (重なりピクセル数 / ポリゴンピクセル数)`で比率から算出。
- 2D陸地パスは`state.landPaths2D`にキャッシュ（`collectLandPaths2D`で`collectLandVectorPaths`と同じサンプリングだが3D変換せず2D座標を保持）。
- 単位: SVG座標値がそのまま赤道長に対応（ユーザーがm/kmを付与）。
